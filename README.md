...
企业内部多个业务系统在关键事件发生时（如用户注册、付款成功、商品购买），需要调用外部供应商的 HTTP(S) API 进行通知。不同供应商的 API 地址和格式各不相同。

核心矛盾在于：**业务系统只管发通知，不应该关心外部 API 是否可达、格式如何、失败了怎么办。**

本系统的定位是**投递通道**——接收业务系统的通知请求，持久化后异步投递到目标地址，处理失败重试，让业务系统与外部供应商解耦。
...
> 详细设计文档见 `docs/` 目录（9 篇），AI 使用说明见 `docs/09-AI使用说明.md`。
...
| 文档 | 内容 |
|------|------|
| 01-系统边界 | 做什么、不做什么、设计原则 |
| 02-可靠性与失败处理 | 投递语义、失败策略、状态流转 |
| 03-取舍与演进 | 不采纳的过度设计、演进路线 |
| 04-Ingestion-API模块设计 | 接口设计、幂等设计 |
| 05-Task-Store模块设计 | 表结构、SKIP LOCKED、PG 选型 |
| 06-Dispatcher模块设计 | 轮询、投递、重试、退避 |
| 07-Statistics模块设计 | 统计接口、SQL 实现 |
| 08-中间件选型说明 | 技术栈选型与替代方案 |
| 09-AI使用说明 | AI 帮了什么、没采纳什么、自己决策了什么 |
